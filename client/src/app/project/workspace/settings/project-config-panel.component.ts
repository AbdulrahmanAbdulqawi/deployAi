...
@Component({
  selector: 'app-project-config-panel',
  standalone: true,
  imports: [FormsModule, RepoFolderPickerComponent, ButtonComponent, IconComponent],
  templateUrl: './project-config-panel.component.html',
  styleUrl: './project-config-panel.component.scss'
})
export class ProjectConfigPanelComponent implements OnInit {
  ...
  isMisconfiguredRailwayTarget(target: TargetConfig): boolean {
    ...
    const isFrontendFolder = ProjectConfigPanelComponent.FrontendDirectoryNames.includes(folderName);
