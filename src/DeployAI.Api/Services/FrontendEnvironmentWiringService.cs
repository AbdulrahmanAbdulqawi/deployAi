...
public sealed class FrontendEnvironmentWiringService : IFrontendEnvironmentWiringService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // A single-origin compose app is one target wearing both hats: the provider-name fallbacks
        // in pair resolution match it as website AND server, and the sync then ran Railway-shaped
        // work against a Coolify uuid — throwing after a successful deploy and flipping the whole
        // deployment to failed. One target has nothing to cross-wire.
        //
        // Filtered per pair rather than returning early, because a project can declare a compose
        // pair alongside a genuine split-origin pair; bailing on the first would leave the second
        // permanently un-synced.
        var wirablePairs = pairs.Where(pair => pair.Website.Id != pair.Server.Id).ToList();
        if (wirablePairs.Count == 0)
        {
            return SkippedResult(
                options.Source,
                completedAt,
        ...
        foreach (var pair in wirablePairs)
        {
